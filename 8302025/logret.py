...
if __name__ == '__main__':
    ...
    dates = dates_obj.date_range(start = dates_obj.trading_day_offset(dte,-1), end = dates_obj.trading_day_offset(dte,5))
    ...
    for s in sids:
        ...
        daily_log_ret = daily_log_ret[daily_log_ret['date'] == dte]

        results.append(daily_log_ret)

    # Combine all stocks into one DataFrame
    final_df = pd.concat(results, ignore_index=True)

    Path(f"{output_directory}/").mkdir(parents=True, exist_ok=True)
    final_df.to_csv(f"{output_directory}/{dte.strftime('%Y%m%d')}.logret.csv",index = False, header = True)  
